...
public class mapZtoSynthParams_Eau3 : MonoBehaviour {
	...
	void Update () {
		...
		if (dist >= distLim && canBeChanged) {
			...
			this.transform.Find("Sphere_Eau3").GetComponent<ShepardRissetBarberpole_Eau3> ().bandwidth = newOctavesNumber;
		...
		this.transform.Find("Sphere_Eau3").GetComponent<ShepardRissetBarberpole_Eau3> ().lowerBound = 13.75f * Mathf.Pow (2f, pow);
		...
		this.transform.Find("Sphere_Eau3").GetComponent<ShepardRissetBarberpole_Eau3> ().tonalVsNoisy = _tonalVsNoisy;
