...
public class mapZtoSynthParams_Eau2 : MonoBehaviour {
	...
	void Update () {
		...
		if (dist >= distLim && canBeChanged) {
			...
			this.transform.Find("Sphere_Eau2").GetComponent<ShepardRissetBarberpole_Eau2> ().bandwidth = newOctavesNumber;
		...
		this.transform.Find("Sphere_Eau2").GetComponent<ShepardRissetBarberpole_Eau2> ().lowerBound = 13.75f * Mathf.Pow (2f, pow);
		...
		this.transform.Find("Sphere_Eau2").GetComponent<ShepardRissetBarberpole_Eau2> ().tonalVsNoisy = _tonalVsNoisy;
