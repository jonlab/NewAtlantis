...
public class mapZtoSynthParams_Terre2 : MonoBehaviour {
	...
	void Update () {
		...
		if (dist >= distLim && canBeChanged) {
			...
			this.transform.Find("Sphere_Terre2").GetComponent<ShepardRissetBarberpole_Terre2> ().bandwidth = newOctavesNumber;
		...
		this.transform.Find("Sphere_Terre2").GetComponent<ShepardRissetBarberpole_Terre2> ().lowerBound = 13.75f * Mathf.Pow (2f, pow);
		...
		this.transform.Find("Sphere_Terre2").GetComponent<ShepardRissetBarberpole_Terre2> ().tonalVsNoisy = _tonalVsNoisy;
