...
public class mapZtoSynthParams_Eau1 : MonoBehaviour {
	...
	void Update () {
		...
		if (dist >= distLim && canBeChanged) {
			...
			this.transform.Find("Sphere_Eau1").GetComponent<ShepardRissetBarberpole_Eau1> ().bandwidth = newOctavesNumber;
		...
		this.transform.Find("Sphere_Eau1").GetComponent<ShepardRissetBarberpole_Eau1> ().lowerBound = 13.75f * Mathf.Pow (2f, pow);
		...
		this.transform.Find("Sphere_Eau1").GetComponent<ShepardRissetBarberpole_Eau1> ().tonalVsNoisy = _tonalVsNoisy;
