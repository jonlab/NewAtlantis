...
public class mapZtoSynthParams_Terre3 : MonoBehaviour {
	...
	void Update () {
		...
		if (dist >= distLim && canBeChanged) {
			...
			this.transform.Find("Sphere_Terre3").GetComponent<ShepardRissetBarberpole_Terre3> ().bandwidth = newOctavesNumber;
		...
		this.transform.Find("Sphere_Terre3").GetComponent<ShepardRissetBarberpole_Terre3> ().lowerBound = 13.75f * Mathf.Pow (2f, pow);
		...
		this.transform.Find("Sphere_Terre3").GetComponent<ShepardRissetBarberpole_Terre3> ().tonalVsNoisy = _tonalVsNoisy;
