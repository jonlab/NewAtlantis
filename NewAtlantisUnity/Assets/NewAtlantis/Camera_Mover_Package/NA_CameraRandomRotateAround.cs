...
public class NA_CameraRandomRotateAround : MonoBehaviour {
	...
	void init(){
		...
		myCamera.transform.position = focusPoint.transform.position + rdV;
	...
	void Update () {
		...
		if (transform.position.y < minimalYPositionValue)
			init ();
